...
// Função para realizar a interpolação bilinear em uma imagem
...
int main() {
    ...
    // Para redução de DPI, fazemos uma cópia simples sem interpolação
    for (int y = 0; y < newHeight; ++y) {
        for (int x = 0; x < newWidth; ++x) {
            // Calcula as coordenadas do pixel original correspondentes ao novo pixel (sem interpolação)
            int srcX = (int)(x / scalingFactor); // Apenas faz a "subamostragem"
    ...
    cv::imwrite("../imgs/resized_image_no_interpolation.tif", resizedImage);
    cv::imwrite("../imgs/enlarged_image_bilinear.tif", enlargedImage);
